    ...
    <div className={cn("glass-card-bright rounded-2xl p-8 scroll-reveal kpi-card-shadow relative overflow-hidden", className)}>
      ...
      <div className="flex items-start justify-between relative z-10">
        <div>
          ...
          {deltaText ? <p className={cn("text-xs font-medium mt-2", deltaClassName)}>{deltaText}</p> : null}
        </div>
        <div className={cn("w-14 h-14 rounded-xl flex items-center justify-center", accentClass[accent])}>
          {icon}
        </div>
      </div>
    </div>
